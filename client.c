...
void *client_socket_reader(void *usernr) {
    ...
    while (1) {
        ...
        int corruptionResult = rndCorrupt(users[mynr].username, buffer);
        if (corruptionResult == -1) {
            // Handle corruption failure if needed
             printf("Sending corruption notification to client %s\n", users[mynr].username);
            write(users[mynr].newsockfd, "Your message was corrupted. Please send it again.\n", 49);
            continue; // Skip processing the corrupted message
        }
